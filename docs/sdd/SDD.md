...
| 파일 응답 | `app/api/image/[id]` | 근거 이미지 바이트 | 기록 변경 |
...
| ~~`recurring`~~ · ~~`recurring_emissions`~~ | 반복 항목과 그 발행 이력 | **읽고 쓰는 코드가 없다(ADR-047).** 마이그레이션은 덧붙이기만 하므로 표는 남는다 |
...
번호 045 는 비어 있다 — 자리가 잡히기 전에 거둔 결정이 쓰던 번호이고, 번호는 다시 쓰지 않는다.
...
| ADR-047 | 2026-09-15 | **폐기한 요구의 코드를 지운다.** 반출·반입(`lib/transfer`·`app/api/export`), 반복 항목(`lib/repo/recurring`·`emitDueRecurring`), 나가는 항목 계약(`outboundFields`·`currentOutboundFields`·`OutboundField`)과 그 시험을 삭제한다. 표(`recurring`·`recurring_emissions`)와 `deleteCategory` 는 남는다 | ① ADR-046 대로 계층을 남기기 ② 표까지 지우기 | 요구가 비범위로 내려간 뒤(D-06) 남은 코드는 **무엇을 지키는지 말할 요구가 없다.** 읽는 사람이 그것을 살아 있는 기능으로 오해한다. ②는 마이그레이션이 덧붙이기만 하는 배열이라 할 수 없고, `deleteCategory` 는 살아 있는 `FR-CAT-04`·`FR-CAT-06` 의 시험이 보관 상태를 만들 때 쓴다 | 되살리려면 그 코드를 다시 쓴다. 글은 TC 문서의 취소선 행이 갖고 있다. 표에 쓰는 코드가 없는 상태가 남는다 | 폐기한 요구 중 하나를 다시 범위에 넣을 때 | Active |
| ADR-046 | 2026-09-15 | **넷째 목적지를 '태그'로 좁힌다.** 이름을 설정 → 태그로 바꾸고 태그 관리만 남긴다. 반복 기록·자동 태그 규칙·백업·외부 전송 안내와 태그 보관 아이콘을 화면에서 내린다. **저장·조회 계층은 그대로 둔다**(ADR-014 와 같은 방식) | ① 다섯 구역을 그대로 두기 ② 다른 목적지로 옮기기 ③ 요구까지 폐기하고 계층도 지우기 | 사용자가 그 구역들을 쓰지 않는다며 내려 달라고 했다. ③은 RFP 까지 거슬러 지웠다가 되살릴 때 그 작업을 되풀이해야 한다 — ADR-014 에서 같은 판단을 했다 | **요구 일곱이 비범위로 내려갔다**(RFP v1.3) — `FR-CAT-03`·`FR-CAT-08`·`FR-ENTRY-12`·`FR-ENTRY-16`·`FR-VIEW-10`·`FR-VIEW-11`·`NFR-VIEW-03`. 저장·조회 계층과 그 시험은 남아 있으므로 **요구 없는 코드**가 생긴다. 되살릴 때 쓸 발판이지만, 그 코드가 무엇을 지키는지는 이제 이 행이 유일한 근거다 | 사용자가 그 구역 중 하나를 다시 쓰겠다고 할 때. 되돌리는 비용은 화면 하나다 | Active — **계층 보존 조항은 ADR-047 이 대체** |
...
| ~~`FR-ENTRY-12`~~ | **[폐기 — ADR-046·ADR-047]** 코드가 지워졌다. 표만 남는다 |
| `FR-ENTRY-13` | §4.1 `images` · ADR-007 |
| `FR-ENTRY-15`, `FR-ENTRY-16`, `FR-ENTRY-18`, `NFR-ENTRY-05` | §2 불변조건 2 · §3.3(`gemini.ts` 단일 지점) · ADR-020. 안내는 보내기 직전 입력창에서 뜬다. ~~`FR-ENTRY-16`~~ 은 폐기됐고 나가는 항목 계약도 지워졌다(ADR-047) |
...
| ~~`FR-VIEW-10`~~, ~~`FR-VIEW-11`~~, ~~`NFR-VIEW-03`~~ | **[폐기 — ADR-046·ADR-047]** §5.3 과 그 코드가 지워졌다. ADR-008 은 기록으로 남는다 |
...
| D-02 | 해석 경계가 노출해야 하는 계약의 모양은 무엇인가 | §2 불변조건 1의 검사 방법, `NFR-ENTRY-04` 의 교체 가능성 | Decided | `InterpretProvider`(이름·네트워크 사용 여부·가용성·해석·조회 해석) 하나로 확정. '나가는 항목'은 ADR-047 로 계약에서 빠졌다. 수단을 바꾸려면 이 계약을 구현하고 `PROVIDERS` 에만 넣는다. 검사는 소스 훑기(`TC-ENTRY-36`)와 실행 중 요청 세기(`TC-ENTRY-35`) 둘이다. 반영 위치: §3.3 |
| D-03 | 근거 이미지를 DB 안에 둘 것인가 파일로 두고 참조만 둘 것인가 | `FR-ENTRY-13`, `NFR-ENTRY-05`, 반출 형식 | Decided | 파일로 두고 DB에는 경로만 둔다(ADR-007). 그로 인해 생긴 반출 문제는 ADR-008이 받았다 — 반출 자체는 ADR-047 로 내려갔다. 반영 위치: §4.1 `images` |
...
| D-06 | ADR-046 으로 화면에서 내려간 요구들을 되살릴 것인가, 비범위로 내릴 것인가 | M 요구 둘을 포함한 일곱이 이행되지 않는 상태였다 | Decided | **비범위로 내렸다.** RFP v1.3 §3.2 가 넷을 약속하지 않는 것으로 적고, 일곱 요구가 RFP·PRD·SRS 에서 취소선으로 남았다. 코드와 시험은 ADR-047 로 지웠다. 반영 위치: ADR-046 · ADR-047 · RFP §3.2·§5·§6 · PRD §5·§8 · SRS §2·§3·§10 |
