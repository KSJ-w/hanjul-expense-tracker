...
/**
 * 예산 대비 — FR-VIEW-09.
 * 예산이 없는 분류는 막대를 그리지 않는다. 없음을 0 으로 그리면 초과로 읽힌다(SRS §6).
 */
export function BudgetBars({ rows }: { rows: BudgetRowView[] }) {
  const tokens = useTokens();
  if (rows.length === 0) {
    return <p className="py-8 text-center text-sm text-[var(--ink-3)]">이 달 지출 기록 없음</p>;
  }
  return (
    <ul className="space-y-3">
      {rows.map((r) => {
        const has = r.budget !== null;
        const ratio = has && r.budget! > 0 ? Math.min(r.spent / r.budget!, 1) : 0;
        const over = has && r.spent > r.budget!;
        return (
          <li key={r.name}>
            <div className="flex items-baseline justify-between gap-2 text-sm">
              ...
              <span className="tabular shrink-0 text-xs text-[var(--ink-3)]">
                {won(r.spent)}
                {has ? ` / ${won(r.budget!)}` : ' · 예산 없음'}
              </span>
            </div>
            {has ? (
              <div className="mt-1.5 h-2 w-full overflow-hidden rounded-full bg-[var(--bg)]">
                <div
                  className="h-full rounded-full transition-[width]"
                  style={{
                    width: `${Math.max(ratio * 100, r.spent > 0 ? 3 : 0)}%`,
                    background: over ? tokens['--dir-expense'] : tokens['--cat-1'],
                  }}
                />
              </div>
            ) : (
              <div className="mt-1.5 h-2 w-full rounded-full border border-dashed border-[var(--line)]" />
            )}
          </li>
        );
      })}
